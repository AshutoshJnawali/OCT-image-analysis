...
% [FileName,FilePath] = uigetfile('*.mat','Select BMO points data file');
% total_data = load( fullfile(FilePath,FileName) );
% sloImage=total_data.sloImage;
% x=total_data.BMOs(:,1);
% % x=x';
% y=total_data.BMOs(:,2);
...
% figure,imshow(sloImage);
hold on;
BMOCenterplot=plot(X0_in,Y0_in,'Marker','.','MarkerSize',20,'color','y');
